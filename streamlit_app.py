...
st.caption("إعادة صياغة مقال بأسلوب مبسّط للجمهور العام مع قفل الأرقام وتوحيد العلامة العشرية – نص فقط جاهز للسيو.")
...
st.markdown("---")
st.markdown("**الخطوة 2:** اضبط الإعدادات (اختياري).")
...
max_output_tokens = st.number_input("الحد الأقصى للتوكنات (لكل إخراج)", min_value=500, max_value=8000, value=2000, step=100)

st.markdown("---")
st.markdown("**الخطوة 3:** إعداد مفاتيح OpenAI (من أسرار Streamlit).")
...
run = st.button("▶️ إعادة الصياغة الآن", type="primary")
    ...
    if not raw_text.strip():
        ...
        with st.spinner("يتم المعالجة…"):
            output, info = process_article(
                raw_text, api_key=api_key, base_url=base_url, model=model
            )
        if info["status"] != "ok":
...
st.markdown("---")
st.caption("تنبيه: لا يتم نشر أي شيء تلقائيًا. التطبيق يُنتج نصًا فقط.")
